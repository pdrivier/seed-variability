...
ggplot() +
  # Individual seed points (fainter)
...
```{r}
df_model_avg <- df_surprisal_by_seed %>%
  group_by(n_params, model, step) %>%
  summarise(mean_surprisal = mean(mean_surprisal), .groups = "drop")

df_surprisal_by_seed %>%
  # filter(step %in% c(0, 4, 8, 16)) %>%
  ...
  geom_jitter(size = .5,
             alpha = .5,
             width = .1) +
  geom_point(data = df_model_avg, size = 2.5) +
  scale_x_log10() +
  labs(x = "Parameters",
       y = "Mean Surprisal",
       color = "") +
